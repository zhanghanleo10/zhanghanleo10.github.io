...
- shared pre-backend seam 应把现有 backend-local `VMIMemoryAccessPlan` 演进为 intent/candidate 两阶段 contract，而不是保留平行同义 plan；稳定 IR/interface、candidate adapter 与 proof invalidation 尚未实现。
